...
public class SimpleGame {
	...
	public String convertTime(int seconds){
        ...
        if (seconds < 0){
            ...
            output = hours + ":" + minutes + ":" + remainingSeconds;
        }
		return output;
	}
	...
	public static void main(String[] args) {
        ...
        SimpleGame simpleConverterSumGame = new SimpleGame();
		
		Scanner sc = new Scanner(System.in);
		
		// Ask the user which game to play.
        System.out.println("Which game do you want to play? There are two options, convert time, or calculate sum");
        ...
        do {
            ...
            System.out.println("Enter number 1 or number 2 to choose: ");
            choice = sc.nextInt();

            // If the user enters 1, ask for an integer to convert and call the convertTime method.
            if (choice == 1) {
                System.out.println("You have chosen the convert time game. Enter an integer to convert from seconds to hours, minutes, and seconds: ");
                int seconds = sc.nextInt();
                String result = simpleConverterSumGame.convertTime(seconds);
                System.out.println(result);
                validChoice = true;
            } 
            else if (choice == 2) {  // If the user enters 2, ask for an integer and call the digitsSum method.
                System.out.println("You have chosen the calculate sum game. Enter a non-negative number: ");
                int input = sc.nextInt();
                int result = simpleConverterSumGame.digitsSum(input);
                System.out.println(result);
                validChoice = true;
            } else {
